		...
		public static function read($path='./ecu/site/laogang/ecu1234567-20280601-131706.engy'){
			...
			for($i=0;$i<$file_count;$i++){
				...
				array_push($file_content,array(
					'date'=>date('Y-m-d H:i:s',bindec(substr($one_file_content,0,32))),
					...
					'elec_power'=>array(
						bindec(substr($one_file_content,160,BYTE_2)),
						bindec(substr($one_file_content,176,BYTE_2)),
						bindec(substr($one_file_content,192,BYTE_2)),
						bindec(substr($one_file_content,208,BYTE_2)),
					),
					'temperature'=>array(
						bindec(substr($one_file_content,225,BYTE_2_UNSIGN))/100,
						bindec(substr($one_file_content,241,BYTE_2_UNSIGN))/100,
					...
					'humidity'=>array(
						bindec(substr($one_file_content,384,BYTE_1)),
						bindec(substr($one_file_content,392,BYTE_1)),
					)
				));
